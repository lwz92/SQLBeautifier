...
'''
    VERSION 1.4 : Pull-in the graphical operation interface, support get the
                  source file and target folder path from button, also support
                  text box entry ,and add the conversion log.
'''
...
class Sql_Convert():
    ...
    def set_init_window(self):
        self.init_window.title("sql大小写美化工具_v1.4")  # 窗口名
    ...
    def beautify_sql_case(self):
        # 获取文本框内容
        self.file_path = self.src_data_text.get(1.0, END).strip().replace("\n", "")  # 加.encode() 输入内容为byte类型
        # print("src =",src)
        self.folder_path = self.dest_data_text.get(1.0, END).strip().replace("\n", "")
